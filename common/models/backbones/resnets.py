...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, use_dilation=False):
        if use_dilation:
            last_stride = 1
            dilation = 2
        else:
        ...
        self.layer4 = self._make_layer(block, 512, layers[3],
                                       stride=last_stride, dilation=dilation)
...
def dilated_resnet34(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3], use_dilation=True, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']), strict=False)
    return model
...
def dilated_resnet50(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], use_dilation=True, **kwargs)
    if pretrained:
